...
The app runs entirely locally via a **Gradio UI** that launches in your browser. No deployment or cloud setup needed.
...
- Displaying top-5 predictions with confidence scores and inference time
...
> First run downloads the ViT model (~350MB) from HuggingFace Hub. Cached after that — no re-download.
...
| Feature | Description |
|---------|-------------|
| Upload tab | Drag and drop any image file — classifies instantly on upload |
...
Do not use `http://0.0.0.0:7860` — that's only what Gradio prints in the terminal.
Use `http://localhost:7860` or `http://127.0.0.1:7860` instead.

**Slow first run**
Normal — the model (~350MB) is downloading. It's cached after the first run and loads in seconds.
...
- **Public URL** — set `share=True` in `demo.launch()` to get a shareable public link instantly
- **Deploy** — push to HuggingFace Spaces for free permanent hosting
...
<div align="center">
Made with 🤗 HuggingFace Transformers + Gradio
</div>
